...
std::optional<int> anti_foo(int i) { return std::nullopt; }

tl::expected<int, std::error_code> anti_bar(int i) {
...
main()
{
   ...
   long t1 = std::chrono::duration_cast<std::chrono::nanoseconds>(en -
st).count();
   //
   ...
   long t2 = std::chrono::duration_cast<std::chrono::nanoseconds>(en2 -
st2).count();
   //
   ...
   long t3 = std::chrono::duration_cast<std::chrono::nanoseconds>(en3 -
st3).count();
...
//
static void bench_direct_int(benchmark::State &state) {
...
//
static void bench_optional_int(benchmark::State &state) {
  unsigned N = state.range(0); // get N from benchmark suite
  // double ff = 0;
  for (auto _ : state) {
    double fcost = 0;
...
//
static void bench_expected_int(benchmark::State &state) {
  unsigned N = state.range(0); // get N from benchmark suite
  // double ff = 0;
  for (auto _ : state) {
    double fcost = 0;
...
//
static void bench_expected_int_int(benchmark::State &state) {
  unsigned N = state.range(0); // get N from benchmark suite
  // double ff = 0;
  for (auto _ : state) {
    double fcost = 0;
...
//
static void bench_anti_optional_int(benchmark::State &state) {
  unsigned N = state.range(0); // get N from benchmark suite
  // double ff = 0;
  for (auto _ : state) {
    double fcost = 0;
...
//
static void bench_anti_expected_int(benchmark::State &state) {
  unsigned N = state.range(0); // get N from benchmark suite
  // double ff = 0;
  for (auto _ : state) {
    double fcost = 0;
...
//
static void bench_anti_expected_int_int(benchmark::State &state) {
  unsigned N = state.range(0); // get N from benchmark suite
  // double ff = 0;
  for (auto _ : state) {
    double fcost = 0;
...
// Shared results here:
// https://github.com/TartanLlama/expected/issues/16#issuecomment-864481060
/*
---------------------------------------------------------------------------
